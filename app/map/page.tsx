...
export function MapPageContent() {
  ...
  useEffect(() => {
    ...
    if (locationParam) {
      ...
      setAddress(decodedLocation);

      // Geocode the URL address to set the map center
      (async () => {
        try {
          ...
          if (response.ok && payload.suggestions && payload.suggestions.length > 0) {
          ...
          // Silently fail - map will use browser geolocation as fallback
        }
      })();
    }

    if (businessParam && Object.keys(BUSINESS_PRESETS).includes(businessParam)) {
      setBusinessPreset(businessParam as BusinessPreset);
    }
  }, [searchParams]);
